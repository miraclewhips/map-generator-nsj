<template>
	...
	<div class="overlay top left flex-col gap">
		...
		<div v-if="selected.length" class="selected">
			...
			<Button @click="changeLocationsCaps" class="smallbtn bg-success" title="Change locations cap for all selected" text="Change locations cap for all selected" />
			      <hr/>
	       <div v-for="country of selected" class="line flex space-between" style="gap:5px">
		...
		<label class="xsmallbtn bg-success">
		  <input type="file" @change="locationsFileProcess($event, country)" accept=".json" hidden />
		  Import
		</label>
		<div>
			<CopyToClipboard :selection="[country]" buttonClass="xsmallbtn bg-success" />
		</div>
		<div>
			<ExportToJSON :selection="[country]" :filename="getName(country)" buttonClass="xsmallbtn bg-success" />
		</div>
		<div> 
	...
	<div class="overlay top right flex-col gap">
		...
		<Checkbox v-model:checked="settings.chooseRandomDate" label="Choose random date" v-if="!settings.checkAllDates" />
...
const handleRadiusInput = (e) => {
  ...
  if (!value || value < 1)  settings.radius = 1;
...
async function getLoc(loc, country) {
  return SV.getPanoramaByLocation(new google.maps.LatLng(loc.lat, loc.lng), settings.radius, (res, status) => {
	...
	if ((!settings.checkAllDates && settings.chooseRandomDate) && res.time && !settings.selectMonths && !settings.rejectOfficial) {
      if (!res.time.length) return false;
      const fromDate = Date.parse(settings.fromDate);
      const toDate = Date.parse(settings.toDate);
      let dateWithin = false;

			for (const loc of shuffle(res.time)) {
    ...
    }else if (settings.checkAllDates && res.time && !settings.selectMonths && !settings.rejectOfficial) {
      if (!res.time.length) return false;
      const fromDate = Date.parse(settings.fromDate);
      const toDate = Date.parse(settings.toDate);
      let dateWithin = false;
...
function isPanoGood(pano) {
  ...
  if ((settings.chooseRandomDate || settings.checkAllDates) && !settings.selectMonths && !settings.rejectOfficial) {
